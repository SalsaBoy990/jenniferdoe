<section class="cta-area bg-light-blue mb-2 min-h-435" style="min-height: 435px;">
    <div class="container-fluid content:px-4 mx-auto sm:px-4 md:px-6 lg:px-6 xl:px-9">
        ...
        <div class="flex flex-col sm:flex-col md:flex-col lg:flex-row gap-x-8 sm:gap-x-8 md:gap-x-32 pt-14 items-center lg:items-start" style="padding-bottom: 73px;">
            <div class="border-accent-turquoise relative rounded-lg border bg-white mx-4 sm:mx-0 main-box">

                <div class="absolute papillon-left-position" style="top: 32px;left: 40px;">
                ...
                <div class="absolute papillon-right-position" style="top: 32px;right: 58px;">
                ...
                <h2 class="text-main-blue text-22px font-extrabold uppercase">
                    {{ __('I help you in the solution') }}
                </h2>
                <div class="text-22px text-main-blue pb0-pt-36px" style="padding-bottom: 0px;padding-top: 36px;">
                    {{ __('Call me for appointment:') }}
                </div>
                <div class="text-accent-turquoise text-xl md:text-3xl font-bold">
                    <a href="{{ 'tel:+' . $phoneNumberLink }}" class="hover:border-accent-turquoise inline-block border-b-2 border-transparent">{{ $phoneNumber }}</a>
                </div>
                <div class="text-22px text-main-blue -mb-1 pt-22px" style="padding-top: 22px;">
                    {{ __('E-mail:') }}</div>
                <div class="text-accent-turquoise text-xl md:text-3xl font-bold">
                    <a href="{{ 'mailto:' . $emailAddress }}" class="hover:border-accent-turquoise inline-block border-b-2 border-transparent">{{ $emailAddress }}</a>
                </div>
            </div>
            <div class="qualifications" style="max-width: 681px; padding-top: 42px;">
                <h2 class="text-main-blue text-22px font-extrabold uppercase mb-6 lg:text-left text-center">
                    {{ __('Qualifications & experience') }}
                </h2>

                <div class="grid grid-cols-1 md:grid-cols-2 md:gap-12 justify-items-center md:justify-items-center">

                    <div class="max-w-280" style="max-width: 280px">
                        @foreach ($qualifications as $qualification)
                            @if ($loop->index < 2)
                                <h3 class="font-bold italic text-sm text-main-blue80pc">{{ $qualification->date }}</h3>
                                <div class="leading-snug pb-5 text-lg text-main-blue80pc">{!! $qualification->description !!}</div>
                            @endif
                        @endforeach
                    </div>
